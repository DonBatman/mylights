...
core.register_node("mylights:lamppost", {
...
--once placed it sets the middle and top nodes
	after_place_node = function(pos)
		core.set_node({x = pos.x, y = pos.y + 1, z = pos.z},{name = "mylights:lpost_top"})
		core.set_node({x = pos.x, y = pos.y + 2, z = pos.z},{name = "mylights:lantern_p"})
	end,
--when you dig this removes the 2 nodes above
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		local pos2 = { x = pos.x, y=pos.y + 1, z = pos.z }
		if core.get_node(pos2).name == "mylights:lpost_top" then
			core.remove_node(pos2)
		end
		local pos3 = { x = pos.x, y=pos.y + 2, z = pos.z }
		if core.get_node(pos3).name == "mylights:lantern_p" then
...
--this checks to make sure there is enough space above to place it. without this any blocks above would be replaced
    on_place = function(itemstack, placer, pointed_thing)
        ...
        if core.get_node({x=pos.x, y=pos.y+1, z=pos.z}).name ~= "air" and
           core.get_node({x=pos.x, y=pos.y+2, z=pos.z}).name ~= "air" then
            core.chat_send_player( placer:get_player_name(), "Not enough vertical space to place a lamp post!" )
            return
        end
...
core.register_node("mylights:lpost_top", {
	tiles = {
		"mylights_lanterntb.png",
		"mylights_lanterntb.png",
		"mylights_lanterntb.png",
...
-- after dig this removes the node above and below
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		local pos2 = { x = pos.x, y = pos.y + 1, z = pos.z }
		if core.get_node(pos2).name == "mylights:lantern_p" then
			core.remove_node(pos2)
		end
		local pos3 = { x = pos.x, y = pos.y - 1, z = pos.z }
		if core.get_node(pos3).name == "mylights:lpost" then
			core.remove_node(pos3)
		end
	end,
})
...
core.register_node("mylights:lantern_p", {
	tiles = {
		"mylights_lanterntb.png",
		"mylights_lanterntb.png",
